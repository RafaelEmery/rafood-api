...
| **agent-boundaries.mdc**  | Always (`alwaysApply: true`)                | No migrate/rollback; `create-migration` only with user approval; no git add/commit/push; read-only git OK; **ADRs created only by user**; no deploy unless asked.                                                             |
